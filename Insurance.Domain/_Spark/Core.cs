...
namespace Insurance.Domain
{
    ...
    public partial class Db
    {
        ...
        // enabe identity insert a new record
...
//                ident = "DELETE FROM " + tableName + "; " + ident;
            try
            {
                using (var connection = CreateConnection())
                {
                    using (var command = CreateCommand(ident, connection, null))
                    {
                        // no identity to return
                        command.ExecuteNonQuery();
                    }
                }
            }
            catch (Exception ex) { throw new DbException(ident, null, ex); }
        }

        public void InsertDataTable(string tableName, DataTable dt)
        {
            using (var connection = CreateConnection())
            {
    ...
    public partial class Entity<T> where T : Entity<T>, new()
    {
        ...
        static void InitSelect()
        {
            ...
            {
                cols.AppendFormat("{0}, ", key.Bracket());
        ...
        public virtual void IdentityInsertEnable(bool on)
        {
            db.IdentityInsertEnable(tableName, on);
        }
        ...
        public DataTable ToDataTable(IList<T> data)
        {
            ...
            for (int i = 0; i < props.Count; i++)
            {
                ...
                if (tableColumns.Contains(prop.Name))
            ...
            foreach (T item in data)
            {
                ...
                {
                    if (tableColumns.Contains(props[i].Name))
        ...
        {
            db.InsertDataTable(tableName, dt);
        }
    ...
    public partial class Repository<T> where T : Entity<T>, new()
    {
        ...
        public void IdentityInsertEnable(T entity, bool on = false)
        {
            entity.IdentityInsertEnable(on);
        }

        public virtual void IdentityInsertDataTable(IList<T> list)
        {
            if (list.Count() > 0)
            {
                T entity = list.First();
                IdentityInsertEnable(entity, true);
                entity.InsertDataTable(entity.ToDataTable(list));
                IdentityInsertEnable(entity, false);
            }
        }
    }
